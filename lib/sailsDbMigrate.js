...
function parseSailsConfig(sailsConfig) {
  ...
  if( 
    typeof sailsConfig.datastores !== 'undefined' && 
    sailsConfig.datastores.length > 0 
  ) {
    connection = sailsConfig.datastores[connectionName];
  } else if( 
    typeof sailsConfig.connections !== 'undefined' && 
    sailsConfig.connections.length > 0 
  ) {
...
module.exports = function (args, sails, done) {
  ...
  // which was causing problems when trying to interactively debug an application that calls sails-grunt-task-db-migrate.
